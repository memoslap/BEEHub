...
"""
        ...
        <div class="reliability-panel-body">
            ...
            <div style="margin-bottom: 30px; padding-bottom: 24px; border-bottom: 2px solid; border-image: linear-gradient(90deg, transparent 0%, rgba(201, 162, 39, 0.4) 10%, rgba(212, 180, 74, 0.55) 50%, rgba(201, 162, 39, 0.4) 90%, transparent 100%) 1;">
                <div style="display:flex; gap:16px; flex-wrap:wrap; align-items:center;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:4px;">
                        <span style="font-size:0.78em; color:#a08840; text-transform:uppercase; letter-spacing:0.4px;">Outcome</span>
                        <div style="display:flex; gap:8px; align-items:center; padding-top:2px;">
                            <label class="radio-pill" title="Each project's declared primary outcome.">
                                <input type="radio" name="cmpRole" value="primary" checked onchange="onSelectionChange()"> Primary</label>
                            <label class="radio-pill" title="Each project's declared secondary outcome.">
                                <input type="radio" name="cmpRole" value="secondary" onchange="onSelectionChange()"> Secondary</label>
                            <label class="radio-pill" title="Show primary and secondary side by side.">
                                <input type="radio" name="cmpRole" value="both" onchange="onSelectionChange()"> Both</label>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        /** Which roles the user asked for: 'primary' | 'secondary' | 'both'. */
        function _selectedRole() {{
            const el = document.querySelector('input[name="cmpRole"]:checked');
        ...
        function _getOutcomePrefixes() {{
            // Always role-keyed — the raw-outcome (RT/Accuracy) view was removed,
            // because it only works when constructs match and is meaningless for
            // paradigm-specific outcomes. The selector chooses WHICH declared
            // role(s) to show.
            const want = _selectedRole();
            const present = _rolePrefixesPresent();
            const wanted = (want === 'both') ? ROLE_PREFIXES : [want];
            const out = wanted.filter(r => present.includes(r));
            return out.length ? out : present;   // fall back to whatever exists
        }}
        ...
        function updateCharts() {{
            ...
            // for whichever role(s) are currently shown.
            const noteEl = document.getElementById('roleMappingNote');
            if (noteEl) {{
                const shownRoles = _getOutcomePrefixes();
                const bits = shownRoles
