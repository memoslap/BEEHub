...
class ProjectOverviewGenerator:
    ...
    def _build_paradigm_panel(self, proj_info: Dict) -> str:
        """Return an HTML <div class='paradigm-panel'> block from a description dict.

        Renders whatever rich fields are present; silently skips absent ones.
        Always shows the short description and badges. Adds background, procedure,
        trial structure, design, timing, software, and keywords when available.
        Safe for both the dashboard card and the standalone project HTML.
        """
        short  = proj_info.get('short_description') or proj_info.get('description', '')
        long_  = proj_info.get('long_description', '')
        bg     = proj_info.get('background', '')
        proc   = proj_info.get('procedure', '')
        trial  = proj_info.get('trial_structure', '')
        design = proj_info.get('design', '')
        soft   = proj_info.get('software', '')
        kws    = proj_info.get('keywords', [])
        timing = proj_info.get('timing', {})
        ...
        resp_device = proj_info.get('response_device', '')
        ...
        if n_sessions:
            badge_items.append(('🔁', f'{n_sessions} sessions'))
        badges_html = ''.join(
            f'<span class="char-badge">{icon} {label}</span>'
            for icon, label in badge_items if label and label != 'unknown'
        )

        # ── Short / long description ──────────────────────────────────────
        desc_html = f'<p class="paradigm-text">{short}</p>'
        if long_:
            desc_html += f'<p class="paradigm-text" style="margin-top:8px;color:#4a6a5a;">{long_}</p>'
        ...
        if proc:
            sections_html += f'''
            <div>
                <div class="paradigm-section-title">Procedure</div>
                <p class="paradigm-text">{proc}</p>
            </div>'''

        if trial:
            sections_html += f'''
            <div>
                <div class="paradigm-section-title">Trial Structure</div>
                <p class="paradigm-text">{trial}</p>
            </div>'''

        if design:
            sections_html += f'''
            <div>
                <div class="paradigm-section-title">Design</div>
                <p class="paradigm-text">{design}</p>
            </div>'''

        # Timing + Software side by side if both present
        meta_items = []
        if timing:
            timing_chips = ''.join(
                f'<span class="timing-item"><span>{k.replace("_", " ").title()}:</span> {v}</span>'
                for k, v in timing.items()
            )
            meta_items.append(f'''
            <div>
                <div class="paradigm-section-title">Timing</div>
                <div class="timing-grid">{timing_chips}</div>
            </div>''')
        if soft:
            meta_items.append(f'''
            <div>
                <div class="paradigm-section-title">Software</div>
                <p class="paradigm-text">{soft}</p>
            </div>''')
        if resp_device:
            meta_items.append(f'''
            <div>
                <div class="paradigm-section-title">Response Device</div>
                <p class="paradigm-text">{resp_device}</p>
            </div>''')
        sections_html += ''.join(meta_items)

        # Keywords
        if kws:
            chips = ''.join(f'<span class="keyword-chip">{k}</span>' for k in kws)
            sections_html += f'''
            <div class="paradigm-full">
                <div class="paradigm-section-title">Keywords</div>
                <div class="keyword-list">{chips}</div>
            </div>'''

        grid_html = ''
        if sections_html:
            grid_html = f'<div class="paradigm-panel-grid">{sections_html}</div>'
