...
class InteractiveDashboard:
    ...
    @staticmethod
    def _jdumps(obj, **kwargs):
        """json.dumps that converts NaN/Inf to null (valid JSON)."""
        def _clean(o):
    ...
    def generate_dashboard_html(self) -> str:
        ...
        projects_json = self._jdumps(self.all_projects, indent=2)
        ranges_json   = self._jdumps(ranges)
...
"""
    ...
    <div class="reliability-panel" id="reliabilityPanel">
        ...
        <div class="chart-container full-width-container" style="margin-bottom:24px;">
            <div class="chart-title">Paradigm Comparison &mdash; role-level (each project's own hierarchy)</div>
            <div style="display:flex; gap:16px; align-items:center; flex-wrap:wrap; margin:8px 0 4px 0;">
                <label class="radio-pill"><input type="radio" name="cmpRole" value="primary" checked onchange="renderParadigmComparison()"> Primary outcomes</label>
                <label class="radio-pill"><input type="radio" name="cmpRole" value="secondary" onchange="renderParadigmComparison()"> Secondary outcomes</label>
            </div>
            <div style="font-size:0.78em;color:#a08840;margin-bottom:10px;">
                Every project declares which of its own outcomes is <b>primary</b> and which is <b>secondary</b>
                (<code>main_metrics</code> in its description JSON). We then compare like with like:
                each project's primary outcome against every other project's primary outcome, across the whole
                metric family &mdash; <i>even though the underlying measures are in different units</i>
                (ms, % correct, a Likert composite). Reliability, sensitivity, internal consistency and drift
                are unit-free, so they share axes; the raw values stay visible in the table below.
            </div>
            <div id="paradigmScatter"></div>
            <div id="paradigmTable" style="margin-top:16px;"></div>
        </div>
    ...
    <script>
        ...
        function _getOutcomePrefixes() {{
            // Collect the set of project-specific outcome IDs flagged as
            // is_global=false in any project's outcome_measures.  These are
            // paradigm-specific measures (e.g. FLOWIDX, DIFF) that don't make
            // sense to compare across projects on the dashboard radar.
            const projectSpecific = new Set();
            (typeof allProjects !== 'undefined' ? allProjects : []).forEach(p => {{
                (p.outcome_measures || []).forEach(om => {{
                    if (om.is_global === false) {{
                        projectSpecific.add((om.id || '').toLowerCase());
                    }}
                }});
            }});
            ...
            Object.keys(DATA_RANGES).forEach(k => {{
                ...
                if (m && !projectSpecific.has(m[1])) prefixes.add(m[1]);
            }});
        ...
        function displayProjects() {{
            ...
            grid.innerHTML = filteredProjects.map(project => {{
                ...
                // Derive headline outcome from outcome_measures (lowest display_priority wins).
                // On the cross-project dashboard, prefer outcomes flagged as
                // is_global=true so paradigm-specific measures (e.g. FLOWIDX)
                // don't dominate the project card.  Project-specific outcomes
                // still appear inside the project's own overview HTML.
                const _allOutcomes = (project.outcome_measures || [])
                    .slice()
                    .sort((a, b) => (a.display_priority || 99) - (b.display_priority || 99));
                const _globalOutcomes = _allOutcomes.filter(om => om.is_global !== false);
                const outcomeMeasures = _globalOutcomes.length > 0 ? _globalOutcomes : _allOutcomes;

                // The project's DECLARED primary main metric wins, when it has one.
                // This is what lets a paradigm-specific measure (FlowIndex_Likert)
                // be the headline without polluting the like-for-like radar:
                // is_global answers "compare RT across paradigms", main_metrics
                // answers "how good is each paradigm's OWN headline measure".
                const _mainPrimary = _mainMetric(project, 'primary');
                const primaryOm = _mainPrimary
                    ? (_allOutcomes.find(o => o.id === _mainPrimary.outcome_id) || null)
                    : (outcomeMeasures[0] || null);
                const primaryId = _mainPrimary
                    ? _mainPrimary.outcome_id.toLowerCase()
                    : (primaryOm ? primaryOm.id.toLowerCase() : 'accbin');
                const primaryLabel = _mainPrimary
                    ? (_mainPrimary.label || _mainPrimary.outcome_id)
                    : (primaryOm ? primaryOm.label : primaryId.toUpperCase());
                ...
                // When main_metrics is present the ICC has already been resolved
                // server-side for that ONE metric, so we use it directly instead
                // of averaging ICCs across trial-type cells. Averaging across
                // heterogeneous conditions (e.g. FLOW's Boredom/Flow/Overload,
                // which sit at floor/ceiling by design) produces a number that
                // describes no condition in particular.
                let overallIcc, iccType, iccCiText = '';
                if (_mainPrimary && _mainPrimary.icc !== null && _mainPrimary.icc !== undefined) {{
                    overallIcc = Number(_mainPrimary.icc).toFixed(2);
                    iccType    = _mainPrimary.icc_type || 'ICC(A)';
                    const lo = _mainPrimary.icc_ci_low, hi = _mainPrimary.icc_ci_high;
                    if (lo !== null && lo !== undefined && hi !== null && hi !== undefined) {{
                        iccCiText = ` <span style="font-size:0.62em;color:#a08840;">[${{Number(lo).toFixed(2)}}, ${{Number(hi).toFixed(2)}}]</span>`;
                    }}
                }} else {{
                    overallIcc = primaryIccVals2.length > 0
                        ? (primaryIccVals2.reduce((a,b) => a+b) / primaryIccVals2.length).toFixed(2)
                        : 'N/A';
                    iccType = (primaryIccVals2.length > 0 && agreementSeen) ? 'ICC(A)' : 'ICC(C)';
                    iccCiText = (ciLow !== null && ciHigh !== null && primaryIccVals2.length === 1)
        ...
        // ═══════════════════════════════════════════════════════════════════
        //  ABSTRACT PARADIGM COMPARISON
        // ═══════════════════════════════════════════════════════════════════
        //
        //  Raw units (ms, % correct, a Likert composite) are NOT comparable
        //  across paradigms. But the *psychometric properties* of each
        //  paradigm's own headline measure ARE unit-free, so they can be put on
        //  one set of axes no matter what the paradigm measures:
        //
        //    RELIABILITY  ICC(A,1)  — can we measure the same person twice?
        //    SENSITIVITY  |Hedges g| — does the manipulation actually move it?
        //    PRECISION    n paired / CI width — how much should we trust the above?
        //
        //  These are the axes a researcher choosing a paradigm actually cares
        //  about, and every project can supply them.

        const G_CAP = 1.5;   // |g| >= 1.5 scores 1.0 (matches the existing radar convention)

        // The comparable metric family. Every project's main metric supplies
        // these, whatever it actually measures. `norm` maps each onto [0,1]
        // where 1 = better, so heterogeneous metrics can share a radar; `fmt`
        // keeps the RAW value visible in the table, because the normalised
        // number is for shape comparison, not for reporting.
        const CMP_METRICS = [
            {{ id: 'icc_agreement',        label: 'ICC(A,1)',     hint: 'absolute agreement across sessions',
               norm: v => Math.max(0, Math.min(1, v)) }},
            {{ id: 'icc',                  label: 'ICC(C,1)',     hint: 'consistency across sessions',
               norm: v => Math.max(0, Math.min(1, v)) }},
            {{ id: 'pearson_r',            label: "Pearson r",    hint: 'rank/linear stability across sessions',
               norm: v => Math.max(0, Math.min(1, v)) }},
            {{ id: 'cronbach_alpha',       label: "Cronbach α",   hint: 'internal consistency',
               norm: v => Math.max(0, Math.min(1, v)) }},
            {{ id: 'paradigm_effect_size', label: '|g| sensitivity', hint: 'does the manipulation move the measure',
               norm: v => Math.max(0, Math.min(1, Math.abs(v) / G_CAP)) }},
            // Inverted metrics. Rules match reliability_metrics.normalise_for_radar
            // EXACTLY (cohens_d: 1-|d|/2 ; cv: 1-v/50, CV is a PERCENTAGE) so the
            // comparison radar and the existing metric radar cannot disagree.
            {{ id: 'session_shift_d',      label: 'Stability',    hint: 'session-to-session drift, inverted (1 = no drift)',
               norm: v => Math.max(0, Math.min(1, 1 - Math.min(Math.abs(v), 2.0) / 2.0)) }},
            {{ id: 'cv_mean',              label: 'Low CV',       hint: 'within-subject variability (%), inverted (1 = low noise)',
               norm: v => Math.max(0, Math.min(1, 1 - v / 50.0)) }},
        ];

        const _num = v => (v === null || v === undefined || Number.isNaN(Number(v))) ? null : Number(v);

        /** Pull one raw metric value out of a main-metric entry. */
        function cmpValue(mm, metricId) {{
            if (!mm) return null;
            const bag = mm.metrics || mm;      // tolerate the flat legacy shape
            return _num(bag[metricId]);
        }}

        /**
         * Role-level paradigm comparison.
         *
         * Each project declares its OWN hierarchy — which of its outcomes is
         * primary and which is secondary. We then compare like with like:
         * every project's PRIMARY outcome against every other project's PRIMARY
         * outcome, across the whole metric family. Nothing is collapsed into a
         * single score, because reliability, sensitivity, internal consistency
         * and drift are genuinely different properties and a paradigm can be
         * strong on one and weak on another — that trade-off is the finding,
         * not noise to be averaged away.
         */
        function renderParadigmComparison() {{
            const role = document.querySelector('input[name="cmpRole"]:checked').value;

            const rows = [];
            for (const p of filteredProjects) {{
                const mm = _mainMetric(p, role);
                if (!mm) continue;
                const vals = {{}};
                let any = false;
                for (const m of CMP_METRICS) {{
                    const v = cmpValue(mm, m.id);
                    vals[m.id] = v;
                    if (v !== null) any = true;
                }}
                if (!any) continue;
                rows.push({{
                    ...
                    label: mm.label || mm.outcome_id,
                    construct: mm.construct || '—',
                    contrast: cmpValue(mm, 'paradigm_effect_size_contrast'),
                    n: _num((mm.metrics || mm).n_subjects) || 0,
                    derived: !!mm._derived,
                    vals: vals
                }});
            }}

            const scDiv = document.getElementById('paradigmScatter');
            const tbDiv = document.getElementById('paradigmTable');
            if (!rows.length) {{
                scDiv.innerHTML = '<div style="padding:24px;color:#a08840;">No project declares a ' +
                    role + ' outcome with computable metrics.</div>';
                tbDiv.innerHTML = '';
                return;
            }}

            // Only show metric spokes that at least one project actually has.
            const active = CMP_METRICS.filter(m => rows.some(r => r.vals[m.id] !== null));

            // ── Radar: one trace per project, spokes = the metric family ─────
            const palette = ['#e0c65a','#6ee7a0','#64b5f6','#ff8a65','#ce93d8',
                             '#4dd0e1','#ffb74d','#f06292','#a1887f','#90a4ae'];
            const traces = rows.map((r, i) => {{
                const theta = active.map(m => m.label);
                const rad   = active.map(m => {{
                    const v = r.vals[m.id];
                    return v === null ? 0 : m.norm(v);
                }});
                theta.push(theta[0]); rad.push(rad[0]);          // close the polygon
                const c = palette[i % palette.length];
                return {{
                    type: 'scatterpolar', mode: 'lines+markers',
                    name: r.project + ' · ' + r.label,
                    theta: theta, r: rad,
                    line: {{ color: c, width: 2 }},
                    marker: {{ color: c, size: 5 }},
                    fill: 'toself', fillcolor: c.replace('#','rgba(')
                        .replace(/^rgba\((..)(..)(..)$/, (mm_,a,b,cc) =>
                            `rgba(${{parseInt(a,16)}},${{parseInt(b,16)}},${{parseInt(cc,16)}},0.06)`),
                    hovertemplate: '<b>%{{fullData.name}}</b><br>%{{theta}}: %{{r:.2f}} (normalised)<extra></extra>'
                }};
            }});
            const layout = {{
                plot_bgcolor: "rgba(15,13,7,0.97)", paper_bgcolor: "rgba(10,9,5,0.95)",
                font: {{ color: '#d4b44a', size: 12 }}, height: 520,
                polar: {{
                    bgcolor: "rgba(15,13,7,0.97)",
                    radialaxis: {{ visible: true, range: [0,1], tickfont: {{color:'#a08840', size:10}},
                                  gridcolor: 'rgba(201,162,39,0.25)' }},
                    angularaxis: {{ tickfont: {{color:'#d4b44a', size:11}},
                                   gridcolor: 'rgba(201,162,39,0.25)' }}
                }},
                showlegend: true,
                legend: {{ bgcolor:"rgba(20,18,10,0.97)", font:{{color:'#d4b44a', size:10}},
                          bordercolor:"rgba(201,162,39,0.45)", borderwidth:1 }},
                margin: {{ l: 60, r: 60, t: 20, b: 30 }}
            }};
            Plotly.newPlot('paradigmScatter', traces, layout, {{responsive:true}});

            // ── Table: RAW values, one column per metric ─────────────────────
            let html = '<table style="width:100%; border-collapse:collapse; font-size:0.84em;">' +
                '<thead><tr style="color:#c9a227; text-align:left; border-bottom:1px solid rgba(201,162,39,0.4);">' +
                '<th style="padding:6px 8px;">Project</th><th>' +
                (role === 'primary' ? 'Primary' : 'Secondary') + ' outcome</th><th>Construct</th>';
            active.forEach(m => {{
                html += `<th style="text-align:right;" title="${{m.hint}}">${{m.label}}</th>`;
            }});
            html += '<th style="text-align:right;">n</th></tr></thead><tbody>';

            // Rank within each metric column so the reader can see who leads where.
            const best = {{}};
            active.forEach(m => {{
                const scored = rows.map(r => r.vals[m.id] === null ? -1 : m.norm(r.vals[m.id]));
                best[m.id] = Math.max(...scored);
            }});

            rows.forEach(r => {{
                const der = r.derived
                    ? ' <span title="hierarchy not declared — display_priority used" style="color:#a08840;">*</span>' : '';
                html += `<tr style="border-bottom:1px solid rgba(201,162,39,0.15); color:#c8b080;">
                    <td style="padding:6px 8px; color:#e0c65a; font-weight:600;">${{r.project}}${{der}}</td>
                    <td>${{r.label}}</td><td style="color:#a08840;">${{r.construct}}</td>`;
                active.forEach(m => {{
                    const v = r.vals[m.id];
                    if (v === null) {{ html += '<td style="text-align:right;color:#6b5a2a;">—</td>'; return; }}
                    const isBest = Math.abs(m.norm(v) - best[m.id]) < 1e-9;
                    const style = isBest ? 'text-align:right;color:#6ee7a0;font-weight:600;'
                                         : 'text-align:right;';
                    html += `<td style="${{style}}">${{v.toFixed(2)}}</td>`;
                }});
                const warn = (r.n && r.n < 12)
                    ? ' <span title="small paired sample" style="color:#ff8a65;">&#9888;</span>' : '';
                html += `<td style="text-align:right;">${{r.n}}${{warn}}</td></tr>`;
            }});
            html += '</tbody></table><div style="font-size:0.72em;color:#a08840;margin-top:8px;">' +
                'Table shows RAW values; the radar shows the same values normalised to [0,1] (1 = better) so ' +
                'that metrics on different scales can share axes. Green = best in column. ' +
                'Stability and Low&nbsp;CV are inverted (drift and noise are bad). ' +
                '* = hierarchy not declared, display_priority used. &#9888; = fewer than 12 paired subjects.' +
                '<br>No combined score is shown on purpose: a paradigm can be highly sensitive yet unreliable ' +
                'for individual differences (and vice versa). Averaging those into one number would hide exactly ' +
                'the trade-off you need in order to choose a paradigm.</div>';
            tbDiv.innerHTML = html;
        }}
