...
class ProjectOverviewGenerator:
    ...
    def analyze_project(self, project_name: str) -> Dict:
        ...
        # Exclude non-task conditions (control, rest, baseline, fixation) so that
        # dashboard ICC and CV metrics reflect only the cognitive task of interest.
        _EXCLUDE_TYPES = {'control', 'rest', 'baseline', 'fixation', 'fix',
                          'instruction', 'pause', 'break', 'catch', 'null'}
        task_trial_types = [t for t in trial_types
                            if t.lower() not in _EXCLUDE_TYPES
                            and not t.lower().startswith('ctrl')
                            and not t.lower().startswith('rest')]
...
"""
    ...
    def _generate_plots_js(self, proj_name: str, data_by_cond: Dict,
                          ...
                          learning_stage_data: Dict = None) -> str:
...
"""
        ...
        # Radar Chart for reliability metrics - LARGER with more metrics
        if reliability:
            categories = []
            values = []
            
            for trial_type, metrics in reliability.items():
                ...
                # Pearson r (Correlation between sessions)
                ...
                # Cohen's d (inverted - smaller is better for reliability)
                # Normalize: 0 = perfect (d=0), 1 = worst (d=2)
                if metrics.get('rt_cohens_d_mean') is not None:
                    d_abs = abs(metrics['rt_cohens_d_mean'])
                    # Invert: 1 - (d/2) so that 0 maps to 1, and 2 maps to 0
                    reliability_score = max(0, min(1, 1 - (d_abs / 2)))
                ...
                # Coefficient of Variation (inverted - lower is better)
                # Normalize: CV of 0% = 1 (perfect), CV of 50% = 0 (poor)
                if metrics.get('rt_cv_mean') is not None:
                    cv_score = max(0, min(1, 1 - (metrics['rt_cv_mean'] / 50)))
                    categories.append(f'{trial_type} RT Consistency')
                    values.append(cv_score)
                
                if metrics.get('acc_cv_mean') is not None:
                    cv_score = max(0, min(1, 1 - (metrics['acc_cv_mean'] / 50)))
        ...
        Plotly.newPlot('{proj_name}_radar', radarData, radarLayout, {{responsive: true}});
"""
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .header {{
            ...
            border-radius: 15px;
            margin-bottom: 40px;
            box-shadow: 0 8px 32px rgba(64,158,128,0.15), inset 0 1px 0 rgba(255,255,255,0.9);
        ...
        .header::before {{
            ...
            height: 4px;
            background: linear-gradient(90deg, #2d8659 0%, #40e0d0 30%, #409e80 60%, #2d8659 100%);
        }}

        .project-name {{
            background: linear-gradient(135deg, #1e5f44 0%, #409e80 40%, #40e0d0 60%, #2d8659 100%);
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            background-clip: text;
            font-size: 3em;
        ...
        .project-full-name {{ color: #888; font-size: 1.4em; margin-bottom: 12px; font-style: italic; }}
        .project-description {{ color: #666; font-size: 1.1em; line-height: 1.6; margin-bottom: 20px; }}

        .char-badges {{ display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 4px; }}
        .char-badge {{
            background: linear-gradient(135deg, #f5f5f5 0%, #e8e8e8 100%);
            padding: 8px 16px; border-radius: 20px; font-size: 0.95em;
            border: 1px solid #2d8659; color: #2d8659;
        }}

        .metrics-row {{
            display: flex; gap: 30px; margin-top: 22px;
            padding-top: 18px; border-top: 2px solid #404040;
        }}
        .metric-item {{ color: #1e5f44; font-size: 1.1em; }}
        .metric-label {{ color: #999; font-size: 0.9em; margin-right: 6px; }}
...
"""
        ...
        # ── Reliability explanation panel (full-width, below radar) ──────
        if reliability:
            html += f"""
            <div class="reliability-panel">
                <div class="reliability-panel-header">
                    ...
                    <div class="reliability-panel-subtitle">
                        All scores are normalised to [0, 1] — higher values indicate better reliability
                    </div>
                </div>
                <div class="metric-cards-grid">
                    ...
                    <div class="metric-card">
                        <div class="metric-card-header">
                            ...
                            <div class="metric-card-tagline">Test-retest consistency across sessions (task trials only)</div>
                        </div>
                        <div class="metric-card-body">
                            <ul class="metric-card-points">
                                <li>Two-way mixed model, single measures — raters (sessions) are fixed, subjects are random</li>
                                <li>Session mean differences are partialled out of the error term but not penalised in the denominator, yielding a consistency estimate</li>
                                <li>Computed on task trial types only — control, rest, and baseline conditions are excluded</li>
                            </ul>
                            <div class="formula-box">
                                ...
                                <span class="formula-range">Range −1 → 1 &nbsp;·&nbsp; higher = more consistent across sessions</span>
                            </div>
                        </div>
                    </div>
